var FnordMetric = (function() {
  ...
  function formatTimeOfDay(_time) {
    if (_time === null) {
      ...
      return decPrint(time.getHours()) + ':' + decPrint(time.getMinutes()) + ':' + decPrint(time.getSeconds());
    }
  }

  function formatTimeRange(range) {
    if (range < 60) {
      return parseInt(range, 10) + ' sec';
    } else if (range < 3600) {
      return parseInt(range / 60, 10) + ' min';
    } else if (range == 3600) {
      return '1 hour';
    } else if (range < (3600 * 24)) {
      return parseInt(range / 3600, 10) + ' hours';
    } else if (range == (3600 * 24)) {
      return '1 day';
    } else {
      return parseInt(range / (3600 * 24), 10) + ' days';
    }
  }
  ...
  widgetFunctions['ToplistWidget'] = function() {
    ...
    function render(opts) {
      ...
      function loadGaugeByRel() {
        loadGauge($(this).attr('rel'));
      }
      for (var k in opts.gauges) {
        headbar.append(
        $('<div></div>').attr('class', 'button mr').attr('rel', k).append(
        $('<span></span>').html(opts.gauges[k].title)).click(loadGaugeByRel));
        if (first) {
          first = false;
          loadGauge(k);
        }
      ...
      if (opts.autoupdate) {
        var secs = parseInt(opts.autoupdate, 10);
        if (secs > 0) {
      ...
      function renderGauge(gkey, gdata) {
        ...
        $(gdata.values).each(function(n, _gd) {
          var _perc = (parseInt(gdata.values[n][1], 10) / parseFloat(gdata.count)) * 100;
          ...
          $('.value', _item).html(formatGaugeValue(gkey, parseInt(gdata.values[n][1], 10)));
  ...
  widgetFunctions['HtmlWidget'] = function() {
    function render(opts) {
      opts.elem.append(
      $('<div class="headbar small"></div>').html(opts.title)).css({
        'marginBottom': 20,
        'overflow': 'hidden'
      });
      var container = $('<div></div>').addClass('html_container').html(opts.html);
  ...
  widgetFunctions['NumbersWidget'] = function() {
    function render(opts) {
      opts.elem.append(
      $('<div class="headbar small"></div>').html(opts.title)).css({
        'marginBottom': 20,
      ...
      for (var k in opts.gauges) {
        var gtick = parseInt(opts.gauges[k].tick, 10);
        var gtitle = opts.gauges[k].title;
        var container = $('<div></div>').addClass('numbers_container').addClass('size_' + opts.offsets.length).attr('rel', k).append(
        $('<div></div>').addClass('title').html(gtitle));
        $(opts.offsets).each(function(n, offset) {
          var _off, _nextoff, _sum;
          if (offset[0] == "s") {
            _off = 0;
            _sum = _nextoff = (gtick * parseInt(offset.slice(1), 10));
          } else {
            _sum = 0;
            _off = offset * gtick;
            _nextoff = gtick;
          ...
          container.append(
          $('<div></div>').addClass('number').attr('rel', k).attr('data-offset', _off).attr('data-sum', _sum).attr('data', 0).append(
          $('<span></span>').addClass('desc').html(formatOffset(_off, _nextoff))).append(
    ...
    function updateValues(opts) {
      ...
      values.each(function() {
        var _sum = parseInt($(this).attr('data-sum'), 10);
        var num = this;
        var at = parseInt(new Date().getTime() / 1000, 10);
        var url = FnordMetric.p + '/' + currentNamespace + '/gauge/' + $(this).attr('rel');
        if (_sum > 0) {
          url += '?at=' + (at - _sum) + '-' + at + '&sum=true';
        } else {
          at -= parseInt($(this).attr('data-offset'), 10);
          url += '?at=' + at;
        }
  ...
  widgetFunctions['TimelineWidget'] = function() {
    function render(opts) {
      ...
      function redrawGauge(first_time, gauge) {
        return (function(json) {
          ...
          var series_data = [];
          for (var p in raw_data) {
            series_data.push([parseInt(p, 10) * 1000, raw_data[p] || 0]);
      ...
      function redrawDatepicker() {
        ...
        Highcharts.dateFormat('%d.%m.%y %H:%M', parseInt(opts.start_timestamp, 10) * 1000) + '&nbsp;&dash;&nbsp;' + Highcharts.dateFormat('%d.%m.%y %H:%M', parseInt(opts.end_timestamp, 10) * 1000));
      }
      ...
      function drawLayout() {
        $(opts.elem).append(
          $('<div></div>').attr('class', 'headbar').append(
            $('<div></div>').attr('class', 'button mr').append(
              $('<span></span>').html('refresh')
            ).click(function() { redrawWithRange(); })
          ).append(
            $('<div></div>').attr('class', 'button mr').append(
              $('<span></span>').html('&rarr;')
            ...
            $('<div></div>').attr('class', 'button').append(
              $('<span></span>').html('&larr;')
            ).click(function() { moveRange(-1); })
          ).append(
            $('<h2></h2>').html(opts.title)
      ...
      if (opts.autoupdate) {
        ...
        if (secs > 0) {
          var autoupdate_interval = window.setInterval(function() {
            if ((parseInt(new Date().getTime() / 1000, 10) - opts.end_timestamp) > (opts.include_current ? 0 : opts.tick)) {
  ...
  widgetFunctions['BarsWidget'] = function() {
    function render(opts) {
      var widget_uid = getNextWidgetUID();
      var chart = false;
      var max_y = 0;
      ...
      function redrawGauge(first_time, gauge) {
        return (function(json) {
          ...
          for (var p in raw_data.values) {
            label_data.push(raw_data.values[p][0] || '?');
            series_data.push(parseInt(raw_data.values[p][1] || 0, 10));
            //max_y = Math.max(max_y, raw_data[p]);
          }
          chart = new Highcharts.Chart({
            chart: {
  ...
  widgetFunctions['PieWidget'] = function() {
    function render(opts) {
      var widget_uid = getNextWidgetUID();
      var chart = false;
        ...
        $(opts.gauges).each(function(i, gauge) {
          $.ajax({
            ...
            success: function(_resp) {
              ...
              gauges_left -= 1;
              for (var _tk in resp) {
                gauge_values[gauge] = parseInt(resp[_tk] || 0, 10);
              }
              if (gauges_left === 0) {
      ...
      function redrawChart(first_time, gauge_values) {
        var series_data = [];
        for (var _gkey in gauge_values) {
          series_data.push([
          opts.gauge_titles[_gkey], gauge_values[_gkey]]);
        }
        chart = new Highcharts.Chart({
          chart: {
            renderTo: 'container-' + widget_uid,
      ...
      function drawLayout() {
        ...
        $('<span></span>').html(opts.title))).append(
        $('<div></div>').attr('id', 'container-' + widget_uid).css({
          height: 270
        }));
      }
  ...
  var sessionView = (function() {
    ...
    $('<div class="headbar small"></div>').html('Event Types')).append(typeListElem);
    var feedElem = $('<div class="sessions_feed"></div>').html(
    $('<div class="headbar small"></div>').html('Event Feed')).append(feedInnerElem);
    var sideElem = $('<div class="sessions_sidebar"></div>').html(
    $('<div class="headbar small"></div>').html('Active Users')).append(listElem);
    ...
    function load(elem) {
      eventsPolledUntil = parseInt(new Date().getTime() / 10000, 10);
      elem.html('').append(filterElem).append(feedElem).append(sideElem);
      startPoll();
      loadEventTypes();
    }
    ...
    function addEventType(type, display) {
      ...
      $('<span class="history"></span>').html('history').click(function() {
      ...
      $('<input type="checkbox" />').attr('checked', true).click(function() {
        updateEventFilter();
      })).append(
      $('<span></span>').html(display)).attr('rel', type));
    }
      ...
      return (function(_data, _status) {
        ...
        var passesFiltering = function(event_data) {
            ...
            if (eventsFilter.uncheckedTypes.indexOf(event_data._type) == -1) {
              if (parseInt(v._time, 10) <= eventsPolledUntil) {
        ...
        if (events.length > 0) {
          eventsPolledUntil = parseInt(events[0]._time, 10) - 1;
        }
        ...
        for (var m = maxevents; m < elems.length; m++) {
          $(elems[m]).remove();
        }
        if (pollRunning) {
    ...
    function renderSession(session_data) {
      ...
      if (session_elem.length > 0) {
        ...
        $('<div class="picture"></div>').html(session_picture)).append(
        $('<a href="' + FnordMetric.p + '/' + currentNamespace + '/play/' + session_data["session_key"] + '" class="name"></a>').html(session_name)).append(
        $('<span class="time"></span>').html(session_time)).append(
        $('<span class="history"></span>').html('history').click(function() {
    ...
    function renderEvent(event_data) {
      ...
      if (event_data._session_key && event_data._session_key.length > 0) {
        ...
        var load_usersession = (function() {
          loadEventHistory({
            session_key: __session_key
          });
        });
        var session_data = sessionData[event_data._session_key];
        if (session_data) {
          ...
          if (session_data._picture) {
            event_picture = $('<div class="picture"></picture>');
            event_picture.append(
            $('<img width="40" />').attr('src', session_data._picture)).click(load_usersession);
          }
        }
      }
      feedInnerElem.prepend(
      $('<li class="feed_event"></li>').append(event_time).append(event_picture).append(event_message).append(event_props));
    }
  ...
  var dashboardView = (function(dashboard_name) {
    ...
    function renderWidget(wkey) {
      ...
      var fn = widgetFunctions[widget.klass];
      if (fn) fn().render(widget);
    }
